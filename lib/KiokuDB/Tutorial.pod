...
All object deconstruction in L<KiokuDB> is done in terms of a typemap.

The typemap contains mappings keyed by class. By default anything that is not
in the typemap and isn't a L<Moose> object is rejected, because there are very
few guarantees regarding that object's storability.

Adding new typemap entries is very easy, and allows you to store arbitrary
objects in L<KiokuDB> as long as you describe how.
